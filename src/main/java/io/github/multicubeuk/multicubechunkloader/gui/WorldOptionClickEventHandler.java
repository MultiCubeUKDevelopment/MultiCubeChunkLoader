...
public class WorldOptionClickEventHandler implements IconMenu.OptionClickEventHandler {
    private MultiCubeChunkLoader plugin;

    public WorldOptionClickEventHandler() {
        super();
    ...
    public void onOptionClick(IconMenu.OptionClickEvent event) {
        ...
        List<ChunkLoader> chunks = plugin.getChunkLoaders()
                .stream()
                .filter(
                        c -> c.getOwner().equals(uuid) &&
                                c.getChunkType() == ChunkLoader.ChunkType.WORLD
        ...
        int balance = pi.getWorldChunks() - existingChunks + activeChunkSize;
        if (size > balance && balance != -1) {
            player.sendMessage(String.format(
                            "%sYou do not have enough world chunks available. Available chunks: %s, Required chunks: %s.",
                            ChatColor.RED,
                            balance,
                            size)
            );
